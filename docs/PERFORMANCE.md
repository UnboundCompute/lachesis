...
| 2026-08-21 | `d43e4bc` | Linux `net/ipv4` non-streamed core composition | 22.68 | — | — | 164,437 | 313,693 | ~1.64* | 130 C/header files / ~114k LOC; `run_project(enrich=False)`, one C job, tokens/proofs disabled. This includes frontend snapshot loading and composition (not directly comparable to the direct-builder rows); exact counts returned. `/usr/bin/time -l` peak RSS 1,760,034,816 bytes. |
| 2026-08-21 | `96e619b` | Linux `net/ipv4` core + pass-2 enrichment in memory | 21.49 | 10.52 | — | 218,376 | 418,665 | ~1.74* | Same 130-file / ~114k LOC tree, one C job, tokens/proofs disabled; `run_project(enrich=False)` followed by `enrich_graph` returned exact enriched counts. `/usr/bin/time -l` peak RSS 1,870,987,264 bytes; dirty checkout, timing/validation datapoint only. |
| 2026-08-21 | `842bb06` | Linux `net/ipv4` cold core + combined pass-2 registry | 30.78 | included | — | 218,344 | 418,729 | ~1.74* | Same one-C-job settings and fresh output; `run_project(enrich=True)` completed with exact enriched graph publication in memory. `/usr/bin/time -l` peak RSS 1,871,380,480 bytes. Compared with the prior 33.07s total row, this is a bounded validation signal (~3% faster) with RSS within measurement noise; dirty checkout counts are not parity claims. |
...
| 2026-08-21 | `8a8282e` | Linux `net/ipv4` cold streamed core + pass-3 overview | 30.74 | 20.04 | 30.74 included | 218,376 | 418,665 | ~1.70* | Fresh Python 3.11/Kùzu disposable runtime, 1 GiB pool, one query thread, tokens/proofs disabled. Store published/reopened, ephemeral overview returned exact canonical counts and 172 sources / 13 sinks; build peak RSS 664,961,024 bytes and query peak RSS 1,826,586,624 bytes. Temporary store/runtime removed after validation. |
| 2026-08-21 | `8149249` | Linux `net/ipv4` cold streamed core + manifest-only overview | 31.55 | 15.32 | 31.55 included | 218,376 | 418,665 | ~1.14* | Fresh Python 3.11/Kùzu disposable runtime, same 1 GiB pool and one query thread. Overview returned exact canonical counts and 172 sources / 13 sinks; query peak RSS 1,228,439,552 bytes versus 1,826,586,624 bytes for the full layered overview (~598 MiB lower). Temporary store/runtime removed after validation. |
...
| 2026-08-21 | `9b3f2d3` | Linux `net/ipv4` direct C pass 1 after protobuf collision identity | 15.53 | — | — | 164,437 | 313,693 | ~0.39* | Same 130-file / ~114k LOC workload and settings; exact counts, `/usr/bin/time -l` peak RSS 419,463,168 bytes. This validates the C collision path; timing/RSS are effectively unchanged from the nearby baseline, not an improvement claim. |
