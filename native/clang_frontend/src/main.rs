...
// Stable IDs end in 20 hexadecimal characters: 80 bits represented by 20
// ASCII bytes. Dedup tables dominate large C builds, especially the three-key
// edge set, so retain the same 80 bits in their packed 10-byte form.
    ...
    if bytes.len() >= 20 {
        ...
        for (index, pair) in suffix.chunks_exact(2).enumerate() {
            match (hex_nibble(pair[0]), hex_nibble(pair[1])) {
                (Some(high), Some(low)) => key[index] = (high << 4) | low,
                _ => { valid = false; break; }
            }
        }
        if valid { return IdKey(key); }
    }
    // External/malformed IDs have no stable suffix. The old representation
    // hashed these too; use the same 80-bit width as ordinary native IDs.
    let mut digest = Sha256::new();
    digest.update(bytes);
    key.copy_from_slice(&digest.finalize()[..10]);
    IdKey(key)
}
...
mod id_key_tests {
    ...
    #[test]
    fn packs_the_stable_hex_suffix_without_losing_bits() {
        assert_eq!(
            id_key("v2:frontend:clang-c:value:00112233445566778899").0,
            [0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99],
        );
        assert_eq!(std::mem::size_of::<IdKey>(), 10);
    }

    #[test]
    fn hashes_ids_without_a_stable_suffix_deterministically() {
        assert_eq!(id_key("external-id").0, id_key("external-id").0);
        assert_ne!(id_key("external-id").0, id_key("different-external-id").0);
    }
}
