...
class GenericRuntimeModel:
    ...
    @staticmethod
    def _template(properties: dict) -> tuple[str, dict] | None:
        callee = str(properties.get("callee") or "").replace("?.", ".")
        method = str(properties.get("method_name") or callee.rsplit(".", 1)[-1])
        if callee in EXACT_MODELS:
            return callee, EXACT_MODELS[callee]
        if method in METHOD_MODELS:
            return f"*.{method}", METHOD_MODELS[method]
        return None
